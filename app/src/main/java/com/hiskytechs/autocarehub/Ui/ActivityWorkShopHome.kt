...
class ActivityActivityWorkShopHome : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mySharedPref= MySharedPref(this@ActivityActivityWorkShopHome)
        workshopId = mySharedPref.getWorkShopDocId()
        binding.apply {

            ar.setOnClickListener {
                startActivity(Intent(this@ActivityActivityWorkShopHome, ActivityWorkshopApproveReq::class.java))
            }
            ...
            add.setOnClickListener {
                startActivity(Intent(this@ActivityActivityWorkShopHome, ActivityAddSparePart1::class.java))
            }
                ...
                .addOnSuccessListener { document ->
                    if (document.exists()) {
                        ...
                        binding.apply {
                            ...
                            //   Glide.with(this@ActivityWorkShopHome).load(profileImageUrl).into(profil)// Assuming you have an extension function to load image
                        }
                    } else {
                        Toast.makeText(this@ActivityActivityWorkShopHome, "User data not found", Toast.LENGTH_SHORT).show()
                    }
                }
                .addOnFailureListener { e ->
                    Toast.makeText(this@ActivityActivityWorkShopHome, "Failed to fetch user data: ${e.message}", Toast.LENGTH_SHORT).show()
                }
        }
    }
    private fun openAddOfferDialog() {
        val dialogView = LayoutInflater.from(this@ActivityActivityWorkShopHome).inflate(R.layout.dialog_add_offer, null)
        val dialog = AlertDialog.Builder(this@ActivityActivityWorkShopHome)
            .setView(dialogView)
            .create()
        ...
        addOfferButton.setOnClickListener {
            ...
            if (title.isNotEmpty() && description.isNotEmpty() && validUntil.isNotEmpty() && imageUri != null) {
                ...
                Toast.makeText(this@ActivityActivityWorkShopHome, "All fields are required", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun openAddNewsDialog() {
        val dialogView = LayoutInflater.from(this@ActivityActivityWorkShopHome).inflate(R.layout.dialog_add_news, null)
        val dialog = AlertDialog.Builder(this@ActivityActivityWorkShopHome)
            .setView(dialogView)
            .create()
        dialog.window?.setBackgroundDrawableResource(android.R.color.transparent) // Set background to transparent
        ...
        ref.putFile(uri).addOnSuccessListener {
            ref.downloadUrl.addOnSuccessListener { downloadUrl ->
                ...
                newsCollection.add(news).addOnSuccessListener {
                    Toast.makeText(this@ActivityActivityWorkShopHome, "News added successfully", Toast.LENGTH_SHORT).show()
                    dialog.dismiss()
                }
            }
        }.addOnFailureListener {
            Toast.makeText(this@ActivityActivityWorkShopHome, "Failed to upload image", Toast.LENGTH_SHORT).show()
        }
    }
        ...
        ref.putFile(uri).addOnSuccessListener {
            ref.downloadUrl.addOnSuccessListener { downloadUrl ->
                ...
                offersCollection.add(offer).addOnSuccessListener {
                    Toast.makeText(this@ActivityActivityWorkShopHome, "Offer added successfully", Toast.LENGTH_SHORT).show()
                    dialog.dismiss()
                }
            }
        }.addOnFailureListener {
